...
- 홈을 중심으로 `About`, `Strengths`, `Experience`, `Projects`, `Skills`, `Contact` 섹션을 한 페이지에 배치한다.
- 메인에서는 별도 `Gallery` 섹션과 Contact 하단 링크 그리드를 두지 않는다. 프로젝트 상세 이동은 `Projects` 카드와 `/work` 라우트에서 처리한다.
...
2. 데스크톱에서 nav 클릭을 확인한다.
   - `PROJECTS`, `SKILLS`, `CONTACT` 또는 대응 섹션 클릭 시 섹션 상단이 약 96px offset 위치로 이동해야 한다.
